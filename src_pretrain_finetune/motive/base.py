...
def load_edges(
    ...
    if pretrain_source:
        ...
        if message == ['message']:
            ...
            s_s_train= s_s_edges.query("subset.isin(@s_s_type)")
            s_s_train = torch.tensor(
                s_s_train[["source_a", "source_b"]].values.T, dtype=torch.long
            ) # torch.Size([2, 50553])

            # Calculate 70% of the size 
            msg_split = int(s_s_train.size(1) * 0.7)  # Compute 70% of 50553

            # select 70% of columns randomly
            msg_indices = torch.randperm(s_s_train.size(1))[:msg_split]
            s_s_msgs = s_s_train[:, msg_indices]

            sup_indices = torch.randperm(s_s_train.size(1))[msg_split:]
            s_s_sup = s_s_train[:, sup_indices]
        ...
        # val_loader and test loader 
            ...
            s_s_sup = s_s_edges.query("subset.isin(@supervision)")
